...
public class AddShopSpawn implements CommandExecutor {

    Main plugin;

    public AddShopSpawn(Main plugin) {
        this.plugin = plugin;
    }
                    ...
                    if (isReallyOnGround(player)) {
                        ...
                        for (String key : cfg.getConfigurationSection("arenas.").getKeys(false)) {
                            if (args[0].equals(key)) {
                                if (!cfg.isSet("arenas." + key + ".shops")) {
                                    cfg.createSection("arenas." + key + ".shops");
                                }
                                if (cfg.getConfigurationSection("arenas." + key + ".shops").getKeys(false).size() < Integer.parseInt(cfg.getString("arenas." + key + ".maxPlayers"))) {
                                    int i = cfg.getConfigurationSection("arenas." + key + ".shops").getKeys(false).size();
                                    cfg.createSection("arenas." + key + ".shops.spawn" + i);
                                    cfg.set("arenas." + key + ".shops.spawn" + i, l);
                                    player.sendMessage(ChatColor.LIGHT_PURPLE + "Le shop " + (i + 1) + " a été créé dans l'arène d'ID " + key);
                                    try {
                                        cfg.save(datasFile);
                                    ...
                                    commandSender.sendMessage(ChatColor.RED + "Vous avez atteint la limite maximale de shop dans cette arène");
                                }
                            }
                        }
                    } else {
